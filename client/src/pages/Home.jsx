...
function Home() {
          ...
          const handleSubmit = async(e) => {
            ...
            try {
              const response = await axios.post('http://localhost:5000/server/user/register', formData);
  ...
  return (
    <div className='bg-gray-300 flex items-center justify-center w-full h-screen'>
      ...
      <label for='firstname' className='grid grid-cols-subgrid col-span-2 text-sm font-bold'>First Name</label>
      <input 
      type='text'  
      id='firstname' 
      ...
      hover:bg-gray-300 transition-all text-black p-2 sm:p-1' placeholder='First Name'
       onChange={handleChange}/>
        ...
        <div className='flex flex-col'>
        <label for='lastname'  className='grid grid-cols-subgrid col-span-2 text-sm font-bold'>Last Name</label>
        <input 
        type='text' 
        id='lastname' 
        className=' border-2 border-black rounded-sm outline-none focus:bg-gray-300
      ...
      hover:bg-gray-300 transition-all text-black p-2 sm:p-1' placeholder='Address' 
       onChange={handleChange}/></div>
        ...
        <div></div>
        <button 
        className='bg-black text-white rounded-md p-1 sm:mt-10 text-center w-3/4
         mx-auto text-sm sm:grid sm:grid-cols-subgrid sm:col-span-2 sm:pl-[170px]'
